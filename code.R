...
library(msm)
library(lubridate)
S_P <- read.csv("C:/Users/Tatyana/Downloads/ВКР/year/S&P.csv")
...
# эта функция нужна для того, чтобы перерассчитывает временные промежутки 
# между переходами
# При генерации для отдельных лет там время каждый раз рассчитывается с нуля
...
# GDP
S_P_GDP <- read.csv("C:/Users/Tatyana/Downloads/ВКР/year/S&P_GDP.csv")
...
S_P_CDS <- read.csv("C:/Users/Tatyana/Downloads/ВКР/year/S&P_CDS.csv")
